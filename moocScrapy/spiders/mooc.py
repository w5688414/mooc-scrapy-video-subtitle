...
class MoocSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in range(len(self.COURSE_IDs)):
            ...
            tmp_download_url = downloadUrls.getDownloadUrl(i)
            ...
            downloadUrls.setDownloadUrl(i,tmp_download_url)

            requests1=scrapy.Request(url=base_infor_url, dont_filter=True, callback=self.infor_parse)
            requests1.meta['item']=i
            requests1.meta['dont_redirect']=True

            yield  requests1
            tmp_url = self.course_page_url + self.course
            requests2=scrapy.Request(url=tmp_url, dont_filter=True, callback=self.parse)

            requests2.meta['item']=i
            requests2.meta['dont_redirect']=True
            yield requests2

    def infor_parse(self, response):
        index=response.meta['item']
        ...
        tmp_download_url = downloadUrls.getDownloadUrl(index)
    ...
    def parse(self, response):
        ...
        request=scrapy.FormRequest(url=self.SOURCE_INFO_URL, formdata=post_data, meta=data,
                                 callback=self.get_course_all_source)
        request.meta['item']=index
        request.meta['dont_redirect']=True
        yield request

    def get_course_all_source(self, response):
        print(response.meta['item'])
        url_index=response.meta['item']
        ...
        tmp_download_url = downloadUrls.getDownloadUrl(url_index)
            ...
            for index, single_chaper in enumerate(chapter_set):
                ...
                for sub_index, single_lesson in enumerate(lesson_set):
                    ...
                    for video_index, single_video in enumerate(video_set):  # 这里item必须放在循环里
                        ...
                        item['download_path']=tmp_download_url
                        ...
                        (getMethod_url, video_id) = self.get_video_getMethod_url(url_index,single_video, video_name)
                    ...
                    for pdf_index, single_pdf in enumerate(pdf_set):
                        ...
                        post_data = self.get_post_data(url_index,single_pdf, pdf_name)
                        if post_data != None:
    ...
    def get_video_getMethod_url(self,url_index, single_content, name, *args):
    ...
    def get_post_data(self,url_index, single_content, name, *args):
