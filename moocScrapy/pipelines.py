...
class MoocscrapyPipeline(object):
    def process_item(self, item, spider):
        ...
        tmp_download_url=item['download_path']
        if item["video_url"]:
            ...
            if item['srt_url']:  # 如果视频有字幕
                file_name=tmp_download_url + 'Videos/' + item['video_name'] + '.srt'
                urllib.request.urlretrieve(item['srt_url'],file_name)  # 字幕下载
            else:
                print('没有字幕,则跳出')
                return item
            ...
            if item['video_type'] == 'hls':  # hls格式视频
                ...
                M3U8 = DownLoad_M3U8(file_url, tmp_download_url,file_name)
                M3U8.run()
            else:  # mp4格式则使用正常文件的多线程下载，线程数设置10
                file_name=tmp_download_url + 'Videos/' + item['video_name'] + '.mp4'
                downloader(item['video_url'],file_name,10).run()
...
class DownLoad_M3U8(object):
    def __init__(self, m3u8_url,tmp_download_url,file_name):
    ...
    def download_single_ts(self, urlinfo):
        ...
        # tmp_download_url = getDownloadUrl()
        tmp_download_url=self.tmp_download_url
